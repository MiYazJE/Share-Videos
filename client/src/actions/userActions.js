...
export const register = (payload, callback) => async (dispatch) => {
    ...
    if (!name || !email || !password)
        return dispatch(addNotification({ msg: 'Fields cannot be empty.', variant: 'error' }));

    dispatch(setLoading(true));
    const { error, nameError, emailError, msg } = await API.register(payload);
    dispatch(addNotification({ msg: msg, variant: !error ? 'success' : 'error' }));
...
export const login = (payload, callback) => async (dispatch) => {
    ...
    dispatch(setLoading(true));
    const {
        info: { msg, nameOrEmailError, passwordError, error },
        user,
