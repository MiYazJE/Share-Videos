...
| Disconnect recovery is unusual | `server/lib/socketIo.js` calls `socket.connect()` for some server-side disconnect reasons | Cleanup/reconnection behavior may not match Socket.IO expectations |
